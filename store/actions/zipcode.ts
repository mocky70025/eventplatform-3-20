...
function getClientIP(): string {
    // In server actions, we can't easily get the client IP
    // This would need to be passed from the client or use a different approach
...
export async function lookupZipcode(zipcode: string) {
    ...
    // Rate limiting by a session identifier (could be enhanced with actual IP)
    const admin = createAdminClient();
    const { data: allowed, error: rlError } = await admin.rpc("check_rate_limit", {
        p_key: `zipcode:${getClientIP()}`,
