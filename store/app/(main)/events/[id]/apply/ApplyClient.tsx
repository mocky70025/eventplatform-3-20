                            ...
                            <div key={field.key}>
                                ...
                                {field.type === "file" ? (
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {(field.options || []).map(opt => {
                                            const cur: string[] = Array.isArray(formAnswers[field.key]) ? formAnswers[field.key] : [];
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    type="button"
                                                    onClick={() => setFormAnswers(prev => {
                                                        const c: string[] = Array.isArray(prev[field.key]) ? prev[field.key] : [];
                                                        const next = c.includes(opt) ? c.filter(v => v !== opt) : [...c, opt];
                                                        return { ...prev, [field.key]: next };
                                                    })}
                                                    className={active
                                                        ? "px-3 py-1.5 rounded-full border text-sm font-medium bg-store-500 text-white border-store-500"
                                                        : "px-3 py-1.5 rounded-full border text-sm font-medium bg-white text-slate-700 border-slate-200 hover:border-store-300"}
                                                >
                                                    {opt}
                                                </button>
                                            );
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex gap-2">
                                            {(field.options || ["あり", "なし"]).map(opt => {
                                                const cur = (formAnswers[field.key] && typeof formAnswers[field.key] === "object") ? formAnswers[field.key] : {};
                                                const active = cur.value === opt;
                                                return (
                                                    <button
                                                        key={opt}
                                                        type="button"
                                                        onClick={() => setFormAnswers(prev => {
                                                            const c = (prev[field.key] && typeof prev[field.key] === "object") ? prev[field.key] : {};
                                                            return { ...prev, [field.key]: { ...c, value: opt } };
                                                        })}
                                                        className={active
                                                            ? "px-4 py-1.5 rounded-full border text-sm font-medium bg-store-500 text-white border-store-500"
                                                            : "px-4 py-1.5 rounded-full border text-sm font-medium bg-white text-slate-700 border-slate-200 hover:border-store-300"}
                                                    >
                                                        {opt}
                                                    </button>
                                                );
                                        ...
                                        <input
                                            type="date"
                                            value={(formAnswers[field.key] && typeof formAnswers[field.key] === "object" ? formAnswers[field.key].date : "") || ""}
                                            onChange={(e) => setFormAnswers(prev => {
                                                const c = (prev[field.key] && typeof prev[field.key] === "object") ? prev[field.key] : {};
                                                return { ...prev, [field.key]: { ...c, date: e.target.value } };
                                            })}
                                            className="bg-slate-50 border border-slate-100 rounded-xl px-4 py-2.5 text-slate-900 outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm"
                                        />
                                    </div>
                                ) : (
                                    <input
