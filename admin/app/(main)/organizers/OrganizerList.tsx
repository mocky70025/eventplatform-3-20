...
export function OrganizerRow({ organizer: initialOrganizer }: { organizer: Organizer }) {
    ...
    return (
        ...
        >
            ...
            <td className="px-6 py-4 whitespace-nowrap">
                <span className={`px-2 py-1 rounded-full text-xs font-bold ${organizer.is_approved ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
            ...
            <div className="flex gap-1 bg-white border border-slate-200 rounded-xl p-1 w-fit">
                {tabs.map(tab => (
                    ...
                    >
                        {tab.label}
                        {tab.count > 0 && (
                            <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${filter === tab.key
